...
LOCALE_DEFAULT = "en-IN"     # used internally; no Locale dropdown
HUMAN_TASK     = "extract"   # reserved for future; no Task Type dropdown
...
/* Make User Role SELECT show black text on white */
div[data-baseweb="select"] > div { background:#ffffff !important; }
div[data-baseweb="select"] * { color:#000000 !important; }
ul[role="listbox"] li { color:#000000 !important; }  /* dropdown items */

/* Make the "Browse files" button text black and button white */
div[data-testid="stFileUploadDropzone"] button { color:#000 !important; background:#ffffff !important; }
div[data-testid="stFileUploadDropzone"] button:hover { background:#f3f4f6 !important; }

/* Improve label visibility on dark background */
label, .st-emotion-cache-1erivf3, .st-emotion-cache-1r6slb0 { color: var(--text) !important; }
label[data-testid='stFileUploadDropzoneLabel'] span { color: var(--text) !important; }
...
def agent_panel():
    # ---- Logos at the very top of the sidebar (side-by-side) ----
...
def metrics_from_current(ocr: dict, final_view: dict):
    """Use FINAL content for counts; keep OCR doc_class & confidence."""
...
def render_app():
    ...
    # Count chip right under the controls
    ...
    with info_col:
        if num_files > 0:
            ...
            st.markdown(
                '<div class="su-card"><span class="su-chip"><span class="su-dot warn"></span>'
                'Upload one or more PNG/JPG images to begin processing</span></div>',
                unsafe_allow_html=True
            )
    ...
    if run:
        ...
        for i, file in enumerate(uploaded_files, start=1):
            ...
            # default final view is sanitized (reinforcement optional later)
    ...
    if st.session_state.results:
        ...
        for idx, res in enumerate(st.session_state.results, start=1):
            # always render from the current FINAL view
            ...
            # Metrics row (based on FINAL view)
            ...
            # Final View (ALWAYS shows current res["final"])
            with tabs[0]:
                ...
                st.download_button("⬇️ Download Final Text",
            ...
            with tabs[idx_reinf]:
                ...
                if submitted:
                    ...
                    try:
                        ...
                        if reinf.get("status") == "ok":
                            # >>> Replace FINAL everywhere
                            res["final"] = reinf.get("final") or res["sanitized"]
                ...
                # Compact audit + optional raw JSON (collapsed)
                reinf = res["reinf"] or {}
                if reinf:
                    ...
                    with st.expander("🔧 View raw model JSON (optional)"):
                        st.json(reinf.get("final") or {})
